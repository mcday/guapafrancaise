...
export function MonologueView({ onEnd }: MonologueViewProps) {
  ...
  useEffect(() => {
    if (store.turns.length === 0 && scenario.openingLineFr) {
      const aiTurn: ConversationTurn = { role: 'ai', text: scenario.openingLineFr, timestamp: Date.now() }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-220px)] lg:h-[calc(100vh-200px)]">
      ...
      {/* Chat area */}
      <div ref={scrollRef} className="flex-1 overflow-y-auto py-3 space-y-2 min-h-0">
